  ...
  return (
    <li>
      <Link
        ...
        onClick={handleClick}
        className={`${
          isItemActive ? "bg-green-800 dark:bg-meta-4" : ""
        } group relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-green-900 dark:hover:bg-meta-4`}
      >
        {item.icon}
        {item.label}
        {item.children && (
      ...
      {item.children && (
        <div
          className={`${
            pageName !== item.label.toLowerCase() ? "hidden" : "block"
