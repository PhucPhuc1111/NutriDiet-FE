...
const MealSelectionForm: React.FC<{ day: Day; editMode: boolean; updateDay: (dayNumber: string, foodDetails: DayFoodDetails) => void }> = ({ day, editMode, updateDay }) => {
  const { data: foodList, isLoading } = useGetAllFoods(1, 100, "");

  const handleChange = (mealType: keyof DayFoodDetails, selectedFoods: string[]) => {
      ...
      {["breakfast", "lunch", "dinner", "evening"].map((mealType) => (
        <Form.Item key={mealType} label={`Bữa ${mealType}`}>
          <Select
            ...
            loading={isLoading}
            onChange={(values) => handleChange(mealType as keyof DayFoodDetails, values)}
          >
            {foodList?.map((food) => (
...
const AddMealDaily: React.FC = () => {
  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between">
        ...
        <Button className="bg-green-800 text-white" onClick={addDay}>Thêm ngày</Button>
      </div>
  ...
  {editMode[day.dayNumber] ? (
    <>
      <Button type="primary" onClick={() => toggleEditMode(day.dayNumber)}>Lưu</Button>
...
<MealSelectionForm day={day} editMode={!!editMode[day.dayNumber]} updateDay={updateDay} />
