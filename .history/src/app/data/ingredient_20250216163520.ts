...
export async function getAllIngredients(pageIndex: number, pageSize: number, search: string): Promise<Ingredient[]> {
  return await request.get(`${baseURL}/api/ingredients
...
export async function updateIngredient(id: string, formData: FormData, token: string) {
  try {
    ...
    const response = await request.putMultiPart(`${baseURL}/api/ingredients
        /${id}`, formData, {
      headers: {
...
export async function deleteIngredientById(id: string, token: string): Promise<void> {
  return await request.deleteWithOptions(`${baseURL}/api/ingredients
    /${id}`, {
    headers: {
      "Authorization": `Bearer ${token}`,
