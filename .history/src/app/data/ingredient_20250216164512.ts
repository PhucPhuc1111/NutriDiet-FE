...
export async function getIngredientById(id: string): Promise<Ingredient> {
  return await request.get(`${baseURL}/api/ingredients/${id}`);
}
export async function createIngredient(formData: FormData, token: string): Promise<Ingredient> {
    try {
      const response = await request.postMultiPart(`${base}/api/ingredients`, formData, {
        headers: {
...
export async function updateIngredient(id: string, formData: FormData, token: string) {
  try {
  
    const response = await request.putMultiPart(`${baseURL}/api/ingredients/${id}`, formData, {
      headers: {
        "Authorization": `Bearer ${token}`,
      }
    });
...
export async function deleteIngredientById(id: string, token: string): Promise<void> {
  return await request.deleteWithOptions(`${baseURL}/api/ingredients/${id}`, {
    headers: {
      "Authorization": `Bearer ${token}`,
