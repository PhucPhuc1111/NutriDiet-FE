...
export async function getDiseaseById(deleteDiseaseById: string): Promise<Allergy> {
...
export async function updateDisease(deleteDiseaseById: string, formData: FormData, token: string) {
  try {
  
    const response = await request.putMultiPart(`${baseURL}/api/disease/${deleteDiseaseById}`, formData, {
      headers: {
        "Authorization": `Bearer ${token}`,
      }
    });
...
export async function deleteDiseaseById(deleteDiseaseById: string, token: string): Promise<void> {
  return await request.deleteWithOptions(`${baseURL}/api/disease/${deleteDiseaseById}`, {
    headers: {
      "Authorization": `Bearer ${token}`,
