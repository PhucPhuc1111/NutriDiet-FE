...
export async function getDiseaseById(deseaseId: string): Promise<Allergy> {
  return await request.get(`${baseURL}/api/disease/${deseaseId}`);
}
...
export async function updateDisease(deseaseId: string, formData: FormData, token: string) {
  try {
  
    const response = await request.putMultiPart(`${baseURL}/api/disease/${deseaseId}`, formData, {
      headers: {
        "Authorization": `Bearer ${token}`,
      }
    });
...
export async function deleteDiseaseById(deseaseId: string, token: string): Promise<void> {
  return await request.deleteWithOptions(`${baseURL}/api/disease/${deseaseId}`, {
    headers: {
      "Authorization": `Bearer ${token}`,
