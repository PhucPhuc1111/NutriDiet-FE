...
export async function getDiseaseById(des: string): Promise<Allergy> {
  return await request.get(`${baseURL}/api/disease/${des}`);
}
...
export async function updateDisease(des: string, formData: FormData, token: string) {
  try {
  
    const response = await request.putMultiPart(`${baseURL}/api/disease/${des}`, formData, {
      headers: {
        "Authorization": `Bearer ${token}`,
      }
    });
...
export async function deleteDiseaseById(des: string, token: string): Promise<void> {
  return await request.deleteWithOptions(`${baseURL}/api/disease/${des}`, {
    headers: {
      "Authorization": `Bearer ${token}`,
