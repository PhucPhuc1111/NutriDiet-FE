...
export async function getAllAllergies(pageIndex: number, pageSize: number, allergyName: string): Promise<Allergy[]> {
  return await request.get(`${baseURL}/api/allergy?pageIndex=${pageIndex}&pageSize=${pageSize}&allergyName=${allergyName}`);
}
...
export const useGetAllAllergies = (
  ...
  return useQuery({
    queryKey: ["allergies", pageIndex, pageSize, allergyName],
    queryFn: () => getAllAllergies(pageIndex, pageSize, allergyName),
    ...config,
  });
}
