...
export async function getDiseaseById(de: string): Promise<Allergy> {
  return await request.get(`${baseURL}/api/disease/${de}`);
}
...
export async function updateDisease(de: string, formData: FormData, token: string) {
  try {
  
    const response = await request.putMultiPart(`${baseURL}/api/disease/${de}`, formData, {
      headers: {
        "Authorization": `Bearer ${token}`,
      }
    });
...
export async function deleteDiseaseById(de: string, token: string): Promise<void> {
  return await request.deleteWithOptions(`${baseURL}/api/disease/${de}`, {
    headers: {
      "Authorization": `Bearer ${token}`,
