...
import { Table, Space, Input, Pagination } from "antd"; // Import Pagination here
...
const AllergyPage: React.FC = () => {
  ...
  // Assuming response is structured with a 'data' property that holds the allergy list
  const allergies = response ? response.data : [];

  // Memoize filteredData calculation based on allergies and searchTerm
  const filteredData = useMemo(() => {
    ...
    return _(allergies)
      ...
      .value();
  }, [allergies, searchTerm]);  // Memoize based on allergies and searchTerm
    ...
    <DefaultLayout>
      <div className="p-4">
        ...
        <Table<Allergy>
          ...
          loading={isLoading}
          pagination={false}  // Disable table pagination here
        />
        ...
        <Pagination
          ...
          total={response?.total || 100}   // Use total from the response (if available)
        />
      </div>
    </DefaultLayout>
  );
