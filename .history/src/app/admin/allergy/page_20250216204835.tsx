...
const AllergyPage: React.FC = () => {
  ...
  const { data: allergies, isLoading } = useGetAllAllergies(currentPage, pageSize, "");
  console.log(allergies);
  
  if (Array.isArray(allergies)) {
    // Nếu allergies là mảng, thì bạn có thể sử dụng các phương thức mảng
    const filteredData = allergies.filter((allergy) =>
      allergy.allergyName?.toLowerCase().includes(searchTerm.toLowerCase())
    );
    console.log(filteredData); // In kết quả đã lọc
  } else {
    console.error("Dữ liệu allergies không phải là mảng");
  }
