...
const AllergyPage: React.FC = () => {
  ...
  const { data, isLoading, isError } = useGetAllAllergies(currentPage, pageSize, "");

  const allergies = data?.data || [];  // Lấy dữ liệu từ trường 'data' trong phản hồi
  ...
  return (
    <DefaultLayout>
      <div className="p-4">
        ...
        <Table<Allergy>
          columns={columns}
          dataSource={allergies} // Truyền đúng dữ liệu vào bảng
          loading={isLoading}
          rowKey="allergyId" // Đảm bảo mỗi dòng có key duy nhất
          pagination={false} // Tắt phân trang của bảng ở đây
        />
        ...
        <Pagination
          ...
          total={100} // Giả sử tổng cộng là 100, bạn có thể thay bằng tổng số bản ghi thực tế nếu có
        />
      </div>
    </DefaultLayout>
  );
