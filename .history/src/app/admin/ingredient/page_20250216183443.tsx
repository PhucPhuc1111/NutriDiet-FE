...
import { Table, Space, Button, Input, Pagination } from "antd"; // Import Space từ antd
...
const IngredientPage: React.FC = () => {
   ...
   const { data: response, isLoading, isError } = useGetAllIngredients(currentPage, pageSize, searchTerm);
    ...
    const totalIngredients = response?.total:any || 0;
    ...
    <DefaultLayout>
      <div className="">
        ...
        <Pagination
          ...
          total={totalIngredients}  // Use total from the response (if available)
          total={response?.total || 0}  // Use total from the response (if available)
   />
      </div>
    </DefaultLayout>
  );
