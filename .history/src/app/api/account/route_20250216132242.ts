...
import { NEXT_PUBLIC_API_BASE_URL } from "@/services/apiClient";
...
export async function getAllAccount(token: string, pageIndex: number, pageSize: number): Promise<Account[]> {
  return await request.get(`${NEXT_PUBLIC_API_BASE_URL}/api/Account/${pageIndex}/${pageSize}`, {
    headers: {
...
export async function getAccountById(token: string, accountId: number): Promise<Account> {
  return await request.get(`${NEXT_PUBLIC_API_BASE_URL}/api/Account/${accountId}`, {
    headers: {
      'Authorization': `Bearer ${token}`,
    },
  });
...
export async function getDashboardData(token: string, filter: string = 'Day'): Promise<DashboardResponse> {
  return await request.get(`${NEXT_PUBLIC_API_BASE_URL}/api/Admin/dashboard?filter=${filter}`, {
    headers: {
      'Authorization': `Bearer ${token}`,
    },
  });
